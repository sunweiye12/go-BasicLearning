...
func main6() {
	...
	mk := make(chan bool, 2) //创建一个标志位(缓存为2) --> 一共有两个通道,每个人通道空了以后就放入一个元素,当里面有两个元素时说明都为空,则可以结束主线程

	// 开启一个线程执行匿名函数
	go func() { // 函数的功能是不断地从 c1 和 c2 中获取数据直到都为空
		a, b := false, false // 用来标志 c1he c2 中是否还有元素,如果没有元素了就不再就往 mk 中放一个元素,之后不再放
	...
	// 一定要关闭通道,否则在沟道中不断取出数据的线程会一直等待造成死锁
	close(c1)
	close(c2)

	// mk中两个元素都取出来的时候(证明两个对列 c1 和 c2 都为空) -- > 可以结束主线程
	for i := 0; i < 2; i++ {
		<-mk
	}
}
